/**
 * Shared types and constants for browser-safe frontend usage.
 * These were extracted from service files that depend on Node.js-only packages
 * (googleapis, @google/genai, Buffer, process) to prevent them from being
 * bundled into the frontend.
 */

// ─── Figma Types (from services/figma.ts) ───
...
// ─── File Parser Types (from services/fileParser.ts) ───
...
// ─── Instagram Types (from services/instagram.ts) ───
...
// ─── VPS Deployment Types & Data (from services/deployment.ts) ───
...
export function getSystemDeploymentStatus(): DeploymentCheckStatus {
  return {
    ...
    sslReady: true,
    environmentVarsLoaded: true,
    healthStatus: 'healthy',
    activeUptimeSeconds: 86400
  };
}
